...
| 5 | KMS/IAM separation verified in hosted env | **PENDING** | Verifier **built** (#227, docs/49) — matrix logic + selftests green | **Run** the verifier hosted (docs/49) → record the allowed/denied matrix green | human operator | Stop: web/request OR exec role can decrypt; wildcarded KMS; wrong alias |
...
| 11 | Web/request path decrypt DENIED | **PARTIAL** | KMS/IAM verifier (#227) + design boundary (runner-only capability; server-only) | Hosted denial verified (docs/49 matrix + RUN GATE B) | human operator / Sam GO | Load-bearing negative; stop if web/request can decrypt |
...
2. **Run the KMS/IAM separation verifier** (docs/49) (criteria 5, 11).
