...
| `OAUTH_COMPLETION_WORKER_OIDC_AUDIENCE` | server-only, **not secret** | The audience **dedicated** to the completion worker | **Must be EXACTLY `https://idcaddie.com/oauth-completion-worker`.** V3 pins the literal at config time: any other value is refused as `worker_audience_not_dedicated`, and any `vercel.com`-origin value as `worker_audience_is_vercel_default`. |
...
| ~~`VERCEL_OIDC_TOKEN`~~ | injected by Vercel | **Not used in Functions** | Build/local-dev path only. In a Function the platform token comes from Vercel’s request context via the one approved module (`vercel-platform-oidc.ts`); application code never reads a header itself, there is no `@vercel/oidc` dependency and no CLI refresh path (doc 83 §8.4). |
