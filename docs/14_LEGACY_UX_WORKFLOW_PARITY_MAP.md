...
| `company/groups/`, `permissionSync.js` | Tenant/company/group management | needs legacy inspection | needs inspection | n/a | n/a | tenants + organizations + memberships + RLS; context resolved (PR #1/#9) | row-level isolation (222 RLS assertions) vs per-project | **Partial** (`verified` backend; no admin UI) | tenant/group **management UI** + tenant switching | **Yes** (if admins manage groups in-app) | [11] tenant-model row |
...
> **The binding, itemized go/no-go checklist now lives in [17 §5](./17_OMC_PRODUCTION_REPLACEMENT_PARITY_GATE.md)** (with the ~105-row grounded replacement matrix in 17 §4); **[18_OMC_CONFIRMATION_PASS](./18_OMC_CONFIRMATION_PASS.md)** is the working confirmation process (questionnaire/workshop/decision-log) that resolves doc 17's `probably`/`unknown` rows from OMC evidence. This doc's gate is the doctrine; **17 is the binding gate — if they disagree on readiness, 17 wins.** **OMC is a production replacement, NOT a pilot.**
...
4. **No cutover on backend readiness alone.** RLS passing (222 assertions), migrations applied, types generated, **contract audit-on-write live (`0010`)** — none of that is a cutover signal. The signal is **workflow parity**, owner-signed.
