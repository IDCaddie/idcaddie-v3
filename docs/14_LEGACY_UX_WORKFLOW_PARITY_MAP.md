...
| `contracts/create/` | Create a contract | needs legacy inspection (form fields) | save/create | n/a | n/a | none — write **RLS authority exists** (`0004`), path/UI **not built** ([13](./13_CONTRACT_STEWARD_WRITE_DESIGN.md)) | steward-only write (procurement org / tenant editor); audit via DB trigger | **Not-started** | the whole create form + server action + audit | **Yes** | [13]; [11] contracts row |
| `contracts/[id]/` (edit) | Edit a contract | needs legacy inspection | save/edit | n/a | n/a | none — RLS authority exists, path/UI not built | same as create | **Not-started** | edit form + server action + audit | **Yes** | [13] |
...
(read-only subsets). RISK-002 + RISK-016 remain **open**; hard delete blocked; contract writes/audit/UI
not built; **OMC/Flywheel cutover and new paid-customer onboarding remain blocked**.
