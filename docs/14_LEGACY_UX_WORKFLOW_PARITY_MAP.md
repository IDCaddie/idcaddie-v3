...
| `files/page.tsx`, `webapp/.../storage/*` | Contract docs / file storage | **inspected ([16 §0])**: client-only MIME check, Storage `files/{id}` (no tenant path), tokenized public URLs | upload/download | needs inspection | n/a | **DESIGNED ([16](./16_CONTRACT_PDF_AI_EXTRACTION_DESIGN.md), PR #33)** + **`files` metadata schema built (`0012`, PR #34** — columns + same-tenant `contract_id` FK + CHECKs + T33); **no upload/Storage/signed-URL/RLS surface yet** | tenant-bound metadata + same-tenant FK live; `files` RLS still default-deny | **Design + schema** (not surfaced) | the upload/Storage/RLS implementation | **Yes** (if users attach docs) | [11] storage row; [16] |
...
| `company/groups/`, `permissionSync.js` | Tenant/company/group management | needs legacy inspection | needs inspection | n/a | n/a | tenants + organizations + memberships + RLS; context resolved (PR #1/#9) | row-level isolation (186 RLS assertions) vs per-project | **Partial** (`verified` backend; no admin UI) | tenant/group **management UI** + tenant switching | **Yes** (if admins manage groups in-app) | [11] tenant-model row |
...
4. **No cutover on backend readiness alone.** RLS passing (186 assertions), migrations applied, types generated, **contract audit-on-write live (`0010`)** — none of that is a cutover signal. The signal is **workflow parity**, owner-signed.
