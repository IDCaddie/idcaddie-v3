...
| `contracts/create/` | Create a contract | **inspected ([15])**: name\*, status (Draft/Executed/Cancelled/Expired), category, monthlyCost, start/expiry/procurement/renewal dates, notes + Upload-PDF/AI tab | Create Contract; back→list; save→detail | n/a | n/a | **`/contracts/new` create form (PR #31)** on the #30 RLS-gated path, audited (`0010`); RLS is the boundary (not client role) | steward/editor write (procurement org / tenant editor); `tenant_id` server-resolved; audit via DB trigger | **Partial** | legacy `category`/`procurementDate`/`notes`/`poNumber`/`autoRenew`/`monthToMonth`/`commodity_*`/`validated` + PDF/AI tab — **no v3 column/surface** ([15](./15_LEGACY_CONTRACT_FORM_INSPECTION.md)) | **Yes** | [13]/[15]; [11] contracts row |
| `contracts/[id]/` (edit) | Edit a contract | same field set ([15]); legacy edits **inline** on detail | save/edit; **Delete** (NOT ported) | n/a | n/a | **`/contracts/[id]/edit` (PR #31)** — dedicated route (legacy inline), posts to `updateContractAction` (PATCH; `tenant_id` immutable) | steward/editor write; denied save → generic (no enumeration) | **Partial** | same field gaps as create; legacy hard-delete intentionally **not** ported | **Yes** | [13]/[15] |
...
| `/contracts/new` + `/contracts/[id]/edit` | **Create / edit a contract** (PR #31) | `contracts/create/`, `contracts/[id]/` (legacy edits inline) | **Partial** | legacy `category`/`procurementDate`/`notes`/`poNumber`/`autoRenew`/`monthToMonth`/`commodity_*`/`validated` + PDF-upload/AI have **no v3 column/surface** ([15](./15_LEGACY_CONTRACT_FORM_INSPECTION.md)); no delete/archive/link | RLS-gated #30 write path, audited (`0010`); **no client role gate** | **blocker** until the missing fields/PDF-AI reach parity or are `Removed-approved` |
...
2. ~~**Contract create/edit UI**~~ — ✅ **DONE (PR #31), but PARTIAL parity** — `/contracts/new` + `/contracts/[id]/edit` posting to the #30 actions; legacy inspected first ([15_LEGACY_CONTRACT_FORM_INSPECTION](./15_LEGACY_CONTRACT_FORM_INSPECTION.md)); RLS is the boundary (v3 does **not** port legacy's client `user.role` gate), denied save is generic (no enumeration), no delete/archive. **NOT Same** — legacy `category`/`procurementDate`/`notes`/`poNumber`/`autoRenew`/`monthToMonth`/`commodity_*`/`validated` + PDF-upload/AI have no v3 column/surface; the form covers only v3's columns. **Cutover stays blocked** until these gaps are consciously closed or `Removed-approved`.
2b. **Contract-form parity gaps** ← **next (if product wants Same)** — add the missing v3 columns (forward migration + types regen) + form fields, and the PDF/AI + file surface (RISK-002), each inspected against legacy first.
