...
| `contracts/[id]/page.tsx` | View one contract | needs legacy inspection | view; edit (see below) | n/a | needs inspection | `/contracts/[id]` read-only detail (direct columns + org IDs + linked apps) — PR #19/#20 | related-org RLS read | **Partial** | org names; invoices/files; edit | **Yes** | [11] contracts row |
| `contracts/create/` | Create a contract | needs legacy inspection (form fields) | save/create | n/a | n/a | backend write **path exists** (`createContractAction`/DAL, PR #30) on **RLS authority** (`0004`) + audit (`0010`); **UI not built** ([13](./13_CONTRACT_STEWARD_WRITE_DESIGN.md)) | steward-only write (procurement org / tenant editor); `tenant_id` server-resolved; audit via DB trigger | **Not-started (UI)** | the create **form** (backend action + audit done) | **Yes** | [13]; [11] contracts row |
| `contracts/[id]/` (edit) | Edit a contract | needs legacy inspection | save/edit | n/a | n/a | backend write **path exists** (`updateContractAction`/DAL, PR #30); **UI not built** | same as create (PATCH; `tenant_id` immutable) | **Not-started (UI)** | the edit **form** (backend action + audit done) | **Yes** | [13] |
...
| `files/page.tsx`, `webapp/.../storage/*` | Contract docs / file storage | needs legacy inspection | upload/download | needs inspection | n/a | none — Supabase Storage deferred; `files` default-deny | scoped storage policies (when built) | **Not-started** | file upload/view + storage policies | **Yes** (if users attach docs) | [11] storage row |
...
| `/contracts` | Read-only contracts list | `contracts/page.tsx` | **Partial** | no filters/sort/search; no create | related-org RLS read | **blocker** until create + list parity |
| `/contracts/[id]` | Read-only contract detail + linked apps panel | `contracts/[id]/page.tsx` | **Partial** | org names not shown; no invoices/files; no edit; no gantt | related-org RLS read; linked-apps via `0006`/`0009` | **blocker** until edit + gantt + child surfaces parity |
...
1b. ~~**Contract write PATH (backend)**~~ — ✅ **DONE (PR #30)** — server-side DAL + `"use server"` actions on the anon client, gated by the existing RLS, `tenant_id` resolved server-side, audit inherited from `0010` ([13 §4](./13_CONTRACT_STEWARD_WRITE_DESIGN.md)). **Invisible backend improvement — no user-visible workflow changed, no UI.** The create/edit **UI** and its **legacy parity are still MISSING** (item 2).
2. **Contract create/edit UI parity** ← **next** — the create/edit form posting to the PR #30 actions; match the legacy contract form workflow (fields/actions/filters = `needs legacy inspection` first — inspect the running legacy app before claiming **Same**); steward-only writes (write authority + audit + backend path already exist); RLS is the boundary, a denied write surfaces a generic "could not save" (no enumeration); no delete/archive button. [13 §7] tests already landed.
