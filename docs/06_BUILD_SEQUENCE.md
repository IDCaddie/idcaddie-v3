...
### Stage 5b⁸ — Contract-file PDF validation foundation 🟡 (PR #40 — pure lib + tests, table NOT surfaced)
- **Goal:** the locally-testable core of the contract PDF upload path (docs/16 §3): `src/lib/files/pdf-validation.ts` — `validateContractPdf` (server-side non-empty → 25 MiB cap → `.pdf` ext → `application/pdf` MIME → `%PDF-` magic, from the **actual bytes**) + `buildContractFileObjectPath` (server-derived tenant-bound `contracts/{tenant_id}/{file_id}.pdf`; non-UUID/traversal rejected) + `sanitizeDisplayFilename` + the canonical private bucket name `contract-files`. **16 unit tests** (suite 51 → 67).
- **Pure lib only:** no Supabase/service-role/network; **no Storage bucket created, no upload action/route/UI, no signed URLs, no scan/AI, no migration, no RLS change** (`0013`/**T34** still govern file-row INSERT authority). The Supabase Storage bucket + object-RLS live in the `storage` schema the local harness can't host/test → deferred to the hosted path ([20](./20_STAGING_HOSTED_APPLY_AND_CUTOVER_DISCIPLINE.md)); a `storage` shim would fake safety, so it was not added.
- **Still NOT surfaced:** `files` has no DAL/route/UI; RISK-001/002/016 open; **cutover stays blocked**. **Next: the hosted private bucket + Storage object policies + the upload action (user-scoped client + the `0013` insert authority, NO service-role), then signed-URL read + extraction worker / UI.** PR #41 ([21](./21_STORAGE_LOCAL_HARNESS_FEASIBILITY.md)) proved the Storage bucket/object-RLS **can't be tested locally** (no `storage` schema in the plain-Postgres harness) → **verified in hosted staging (doc 20 / 21 §6 checklist); no fake shim.** The reviewed, human-executed staging apply runbook for the bucket is [22_HOSTED_STORAGE_BUCKET_APPLY_RUNBOOK](./22_HOSTED_STORAGE_BUCKET_APPLY_RUNBOOK.md) (PR #42, runbook only — nothing applied).
