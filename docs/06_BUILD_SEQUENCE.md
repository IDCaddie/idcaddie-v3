...
- **Verified:** `npm test` 44 → **51** (+7 unit tests); `next build` routes unchanged; `test-rls.sh` re-applies `0001`–`0011` → **177**; `gen-types-local.sh` updates `database.types.ts` with the 6 columns. RISK-002 + RISK-016 open; **cutover stays blocked**. **Next: remaining gaps (files/AI/gantt) consciously, or the next legacy write workflow.**
