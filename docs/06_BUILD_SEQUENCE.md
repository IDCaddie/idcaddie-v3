...
- **Still NOT surfaced:** `files` has no DAL/route/UI; RISK-001/002/016 open; **cutover stays blocked**. **Next: the hosted private bucket + Storage object policies + the upload action (user-scoped client + the `0013` insert authority, NO service-role), then signed-URL read + extraction worker / UI.**
