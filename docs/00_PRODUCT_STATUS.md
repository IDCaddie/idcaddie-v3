...
## Merged PRs
**PRs #1–#71 are merged** (main @ `eb2d7e1`); **PR #72 (record OMC full-parity scope decision — docs-only decision of record (doc 38): **OMC requires full old-app parity before cutover unless OMC explicitly waives a specific capability in writing** (via the doc 27/17 `removed-approved`/`deprecated-approved`/`not-used-by-OMC` taxonomy + OMC signoff). **The MVP subset framing is not sufficient for OMC cutover** — `v3-product-scope` annotated superseded-for-cutover (kept as history); doc 17/27/38 control scope. AI/connectors/imports/exports/reporting/dashboards/old-app-UI-UX + all critical workflows are in scope (blockers) unless OMC-waived; no implicit deferrals. Includes the **full-parity cutover rule**, the **27 required parity categories**, a full-parity next-PR sequence, and a PR-count warning (**full parity likely means dozens of PRs**, not ~25–40). **Old-app parity / UI-UX / AI-API-connector parity NOT complete; hosted Auth/tenant-context verified but old-app replacement not yet verified; decision builds no feature, waives nothing, ticks no doc 17 box; RISK-001 stays OPEN; cutover BLOCKED; not production-ready; Storage completion is necessary but not sufficient**) is this PR.** The full
