...
repo on 2026-06-17 (PRs through #48 merged — the contract-file Storage paper trail + the staging env-var inventory; PR #47/#48 recorded the first **partial** staging hosted apply: **migrations `0001`–`0013` applied to the STAGING project `ycdpzduxugdsffjqyoai` + the private `contract-files` bucket created** (`public=false`, 25 MiB, `application/pdf`) — [25 §0](./25_STAGING_SCHEMA_AND_STORAGE_APPLY_EVIDENCE.md). #49 is a **docs-only** wording fix: reword [04 · RISK-001](./04_RISK_REGISTER.md) from "nothing applied" to **"hosted apply PARTIAL/INCOMPLETE"** (+ consistency in 00/09/10). **Storage object policies are NOT applied; doc 21 §6 verification NOT complete; production `dzbfxulvxchdemcettrx` NOT touched** → no upload route/action/UI, signed URLs, AI, or OCR built. **RISK-001 stays OPEN** (partial — object policies + verification + production still pending). RLS stays 205; tests 67/67; `files` still not surfaced; cutover stays BLOCKED; RISK-001/002/007/016 open). **OMC is a paying production-replacement customer, NOT a pilot.** Status words are defined in [10_DOCS_INDEX](./10_DOCS_INDEX.md#status-taxonomy).
...
## Merged PRs
**PRs #1–#48 are merged** (main @ `590914f`); **PR #49 (clarify RISK-001 partial staging apply status — docs-only; rewords RISK-001 from "nothing applied" to "hosted apply PARTIAL/INCOMPLETE"; no hosted mutation, production untouched, RISK-001 stays OPEN) is this PR.** The full
...
| Item | Status |
|------|--------|
| Migrations `0001`–`0013` (core schema, org RLS, related-org read, delete hardening, child integrity, org-scoped child reads + tenant-bind hardening, contract audit-on-write, contract form parity fields, files metadata foundation, files RLS policies) | `implemented`, `verified-local`, `ci-enforced`; **`staged`** (applied to staging `ycdpzduxugdsffjqyoai` — PR #47/#48); **not** `production-applied` |
...
| Migrations applied to hosted Supabase (staging/prod) | **staging done** (`0001`–`0013` `staged` to `ycdpzduxugdsffjqyoai` — PR #47/#48; Storage object policies + verification still pending); **production not done**. RISK-001 OPEN (partial — [04](./04_RISK_REGISTER.md)) |
