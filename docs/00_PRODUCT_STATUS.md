...
## Merged PRs
**PRs #1–#79 are merged** (main @ `827022d`); **PR #80 (record contract-file cleanup disposition — docs-only (doc 41 §15): a human attempted to clean up the old pre-fix synthetic-test.pdf staging rows (delete from `storage.objects` directly + matching `public.files` rows). **Direct SQL cleanup of `storage.objects` was safely blocked by Supabase Storage protections** (*"Direct deletion from storage tables is not allowed. Use the Storage API instead."*) — **no cleanup was performed**; production untouched. **Pre-fix staging contract-file rows remain as historical evidence** (1 good finalized post-PR #78 upload — valid + openable — + 4 old `pending` rows, 2 with objects + 2 orphans). **Direct `storage.objects` DELETE is NOT an acceptable cleanup path; future cleanup must use an approved Storage API/admin/worker path** (request path has no DELETE policy by design). **Storage authorization remains necessary but not sufficient for cutover; upload is not automatically production-ready; Old-app/UI-UX/AI-API-connector parity NOT complete; no doc 17 box ticked; RISK-001 stays OPEN; cutover BLOCKED**) is this PR.** The full
