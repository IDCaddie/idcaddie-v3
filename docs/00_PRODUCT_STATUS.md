...
## Merged PRs
**PRs #1–#77 are merged** (main @ `8c190a2`); **PR #78 (harden contract-file upload finalization — feature hardening (E09a) closing the gap the PR #77 staging finding exposed (3 `files` rows for the contract, 1 orphan with no object, all stuck `pending`; doc 41 §12). PR #76 inserted the row first then uploaded the object, but `0013` had no UPDATE policy + `0015` granted only SELECT/INSERT, so a row could never leave `pending`. **Migration `0016`** (the narrow migration the existing schema required) adds a NARROW uploader-finalize UPDATE policy (`uploaded_by=auth.uid() AND can_write_contract`; WITH CHECK no-reassign) + a **privilege correction**: `revoke update, delete, truncate on public.files from authenticated` then `grant update (upload_status)` (after it `authenticated` holds EXACTLY SELECT, INSERT, UPDATE(upload_status) — no DELETE, no TRUNCATE) — proven by **T36** (row scoping) + **T37** (privilege surface). **Staging verification CAUGHT a broad-grant bug before merge** — the first cut's `grant update (col)` was additive and left `authenticated` with broad DELETE/TRUNCATE/UPDATE on `public.files` (and `test-rls.sh`'s blanket grant had masked it); the corrected `0016` revokes them, the harness now re-asserts the files grants, and a human must re-apply the corrected SQL to staging (not done here). The DAL finalizes success → `upload_status='uploaded'` and dispositions a failed upload → `failed`; download only signs finalized rows; the UI shows Open only for finalized files. **No service-role on request paths, no public bucket, no DELETE/`FOR ALL`; types 0-diff; suite 222→248. Contract-file upload finalization is hardened; successful uploads no longer remain ambiguous pending rows; failed uploads are explicitly dispositioned. Storage authorization remains necessary but not sufficient for cutover; upload is not automatically production-ready; Invoices remain not built; PDF/AI extraction remains not built; Old-app/UI-UX/AI-API-connector parity NOT complete; no doc 17 box ticked; RISK-001 stays OPEN; cutover BLOCKED**) is this PR.** The full
