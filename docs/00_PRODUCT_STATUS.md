...
## Merged PRs
**PRs #1–#82 are merged** (main @ `f1d3de3`); **PR #83 (improve apps inventory/detail parity — UI/read-only (roadmap E03/E04, doc 41 §18): `/apps` now shows per-app **RLS-scoped linked-contract + app-user counts** (new server DAL `listAppsWithCountsForCurrentUser()` — honest "visible to you" tallies; no service-role, no embedded joins) + clearer empty state; `/apps/[id]` adds a clear **"Not built yet" Actions** section (link/unlink, edit/archive, connector sync, AI app/license analysis, export) on top of the existing summary/ownership/linked-contracts/account-intel/roster-with-match-status. No migration/policy/RLS change; no writes; no cross-tenant (RLS-scoped, T25/T28/T29); DTO carries no tenant id/secret/token (tested); types 0-diff; suite 248; tests 83→87. **Apps inventory/detail parity is improved but not complete; App write/edit/delete workflows remain not built; Connector sync remains not built; AI app/license intelligence remains not built; Old-app/UI-UX/AI-API-connector parity NOT complete; hosted Auth/tenant-context verified but old-app replacement not yet verified; upload not production-ready; no doc 17 box ticked; RISK-001 stays OPEN; cutover BLOCKED**) is this PR.** The full
