...
## Merged PRs
**PRs #1–#75 are merged** (main @ `20ec17e`); **PR #76 (add contract file attachment UI — the FIRST build PR (roadmap epic E09a) on the verified Storage boundary: `/contracts/[id]` now lists/uploads/opens PDF attachments, reusing the `files` `0012`–`0015` foundation + the private `contract-files` bucket + the existing RLS/Storage policies + `pdf-validation.ts` — **no migration/schema change**, user-scoped client only (**no service-role on request paths**), **no public bucket**. Files-row-first RLS INSERT (`can_write_contract`, server-derived `tenant_id`/path), 60 s signed URL only after the RLS read, DTO never leaks `storage_path`/URLs; cross-tenant denial proven by `org_rls_test.sql` T34 + the hosted REST verifier 14/14; +9 app-layer tests (76 total). **Contract-file upload/download UI is partially implemented; Storage authorization remains necessary but not sufficient for cutover; upload is not automatically production-ready; Old-app/UI-UX/AI-API-connector parity NOT complete; no doc 17 box ticked; RISK-001 stays OPEN; cutover BLOCKED**) is this PR.** The full
