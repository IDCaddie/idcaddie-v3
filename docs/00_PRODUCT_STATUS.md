...
## Merged PRs
**PRs #1–#73 are merged** (main @ `2bad057`); **PR #74 (create code-derived old-app inventory — docs-only (doc 40) built from the legacy codebase (`frontend-v2/`, `webapp/functions/`, `extension/`, `DemoFeatures/`) since the live app is inaccessible: 50+ frontend routes, backend functions by trigger, **52 connectors/scrapers** (+ IDCIngestor; vault prerequisite RISK-007), AI Doc-processing (contract+invoice), 7 report types, and admin/security (SSO/SAML/OIDC, SCIM, groups+granular permissions, billing, API keys/tokens, audit incl. 90-day purge), each compared to v3 from **repo evidence only** (absent ⇒ Missing) + top blockers + a likely PR backlog + a cannot-determine-without-live-access list. The code reveals a far larger app than prior docs implied — confirming **dozens of PRs**. **Derived from code, NOT live inspection — live old-app inspection remains incomplete; no screenshots/acceptance; no feature built; no doc 17 box ticked; Old-app/UI-UX/AI-API-connector parity NOT complete; hosted Auth/tenant-context verified but old-app replacement not yet verified; RISK-001 stays OPEN; cutover BLOCKED; not production-ready; Storage completion is necessary but not sufficient**) is this PR.** The full
