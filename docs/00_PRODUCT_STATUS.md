...
## Merged PRs
**PRs #1–#78 are merged** (main @ `33053dc`); **PR #79 (record contract-file finalization staging evidence — docs-only (doc 41 §14): a human verified PR #78's hardening on staging. Privilege repair verified — **authenticated no longer has DELETE or TRUNCATE on `public.files` in staging**, SELECT/INSERT=true, **UPDATE only on `upload_status`**, the 3 `files` policies are members-read/writers-insert/uploader-finalizes-own, production untouched. New-upload finalization verified on `https://idcaddie-v3.vercel.app/contracts/cccca1cc-…cc` (synthetic Tenant A): **`upload_status='uploaded'`, `storage_bucket='contract-files'`, `content_type='application/pdf'`, `byte_size=53826`, `has_storage_object=true`** (scan/extraction correctly stay pending/not_started). New uploads now finalize; **existing pre-fix synthetic-test.pdf pending/orphan rows remain as historical staging evidence; no cleanup performed.** Verifies only the new finalization path on staging — not invoices/AI/parity. **Storage authorization remains necessary but not sufficient for cutover; upload is not automatically production-ready; Invoices remain not built; PDF/AI extraction remains not built; Old-app/UI-UX/AI-API-connector parity NOT complete; no doc 17 box ticked; RISK-001 stays OPEN; cutover BLOCKED**) is this PR.** The full
