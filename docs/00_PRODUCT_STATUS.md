...
repo on 2026-06-17 (PRs through #40 merged — contract-file PDF validation foundation `src/lib/files/pdf-validation.ts`; #41 is a **docs-only feasibility spike** ([21_STORAGE_LOCAL_HARNESS_FEASIBILITY](./21_STORAGE_LOCAL_HARNESS_FEASIBILITY.md)): **can Supabase Storage object-RLS be tested locally? Verdict NO** (empirically proven — the plain-`postgres:16` harness has no `storage` schema; SQL-only ≠ storage-api enforcement). No fake shim, no migration, no bucket, no upload action; **Storage object-RLS is verified in hosted staging per doc 20** (doc 21 has the checklist). RLS stays 205; tests 67/67; `files` still not surfaced; cutover stays BLOCKED; RISK-001/002/007/016 open). **OMC is a paying production-replacement customer, NOT a pilot.** Status words are defined in [10_DOCS_INDEX](./10_DOCS_INDEX.md#status-taxonomy).
...
## Merged PRs
**PRs #1–#40 are merged** (main @ `6506cbb`); **PR #41 (Supabase Storage local-harness feasibility spike — docs-only, creates doc 21; verdict: not locally testable → hosted staging) is this PR.** The full
