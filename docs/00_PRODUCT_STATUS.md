...
repo on 2026-06-17 (PRs through #47 merged — the contract-file Storage paper trail + the staging env-var inventory + PR #47's first hosted-apply attempt [agent correctly did NOT execute]; #48 is a **docs/evidence-only** update ([25_STAGING_SCHEMA_AND_STORAGE_APPLY_EVIDENCE](./25_STAGING_SCHEMA_AND_STORAGE_APPLY_EVIDENCE.md) §0): recording that **a human subsequently applied migrations `0001`–`0013` to the STAGING project `ycdpzduxugdsffjqyoai` and created the private `contract-files` bucket** (`public=false`, `file_size_limit=26214400` = 25 MiB, `allowed_mime_types=application/pdf`). **Production `dzbfxulvxchdemcettrx` NOT touched; this PR ran no hosted command. Storage object policies are NOT yet applied** → no upload route/action/UI, signed URLs, AI, or OCR built. **RISK-001 stays OPEN** (staging apply is partial — object policies + verification + production still pending). RLS stays 205; tests 67/67; `files` still not surfaced; cutover stays BLOCKED; RISK-001/002/007/016 open). **OMC is a paying production-replacement customer, NOT a pilot.** Status words are defined in [10_DOCS_INDEX](./10_DOCS_INDEX.md#status-taxonomy).
...
## Merged PRs
**PRs #1–#47 are merged** (main @ `503c39a`); **PR #48 (update staging bucket creation evidence — docs/evidence-only, updates doc 25 §0; records a real human-executed staging apply of `0001`–`0013` + the private `contract-files` bucket; **this PR ran no hosted command, production NOT touched, object policies NOT applied, RISK-001 stays OPEN**) is this PR.** The full
