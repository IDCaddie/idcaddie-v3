...
| Read-only contracts list + detail (`/contracts`, `/contracts/[id]`) | **`partial` — read-only only** (PR #19 — server-rendered, typed DAL `src/lib/data/contracts.ts`, route id lookup-only, related-org RLS). Direct `contracts` columns only; **linked apps / invoices / files intentionally NOT shown** (`app_contracts` tenant-only, invoices/files default-deny — RISK-002). No create/edit/delete/import/export. `verified-local` (RLS spot-check: owner sees both tenant contracts; related-org user sees only its related contract; unrelated org-only + non-member → not_found) |
...
| App–contract linking · invoices · files · license surfaces · writes | `deferred` (child/link tables tenant-only or default-deny — not safe to surface; RISK-002 open) |
| Product UI / app workflows (people, reports, writes) | `planned` (only read-only apps + contracts lists exist) |
