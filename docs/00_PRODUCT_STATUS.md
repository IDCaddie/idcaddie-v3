...
repo on 2026-06-17 (PRs through #50 merged — the contract-file Storage paper trail + the staging env-var inventory; PR #47/#48 recorded the first **partial** staging hosted apply (**migrations `0001`–`0013` applied to the STAGING project `ycdpzduxugdsffjqyoai` + the private `contract-files` bucket created**, `public=false`, 25 MiB, `application/pdf` — [25 §0](./25_STAGING_SCHEMA_AND_STORAGE_APPLY_EVIDENCE.md)); #50 finalized the object-policy plan ([22 §5](./22_HOSTED_STORAGE_BUCKET_APPLY_RUNBOOK.md)). #51 adds **migration `0014`** — the `can_write_contract_file` / `can_read_contract_file` `SECURITY DEFINER` predicates those policies will call (+ T35 RLS tests). **No `storage.objects` policy is applied; no hosted command; doc 21 §6 verification NOT complete; production `dzbfxulvxchdemcettrx` NOT touched** → no upload route/action/UI, signed URLs, AI, or OCR built. **RISK-001 stays OPEN** (partial — object policies + verification + production still pending). RLS now 222; tests 67/67; `files` still not surfaced; cutover stays BLOCKED; RISK-001/002/007/016 open). **OMC is a paying production-replacement customer, NOT a pilot.** Status words are defined in [10_DOCS_INDEX](./10_DOCS_INDEX.md#status-taxonomy).
...
## Merged PRs
**PRs #1–#50 are merged** (main @ `2e255b6`); **PR #51 (add contract-file Storage authorization helpers — migration `0014` only: `can_write_contract_file` / `can_read_contract_file` SECURITY DEFINER predicates + T35 RLS tests; **no Storage object policy applied, no hosted command, production untouched, RISK-001 stays OPEN**) is this PR.** The full
