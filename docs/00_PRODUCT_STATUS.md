...
## Merged PRs
**PRs #1–#72 are merged** (main @ `ab37cd4`); **PR #73 (prepare old-app direct inspection inventory — docs-only inspection packet (doc 39) for full-parity cutover: a 14-field per-screen capture template + a 30-category inventory seeded with the real legacy `frontend-v2/` routes (from `current-product-map.md`) + current v3 status + capture instructions (no secrets/tokens/real customer data) + an OMC interview script + a full-parity gap ledger (the future build backlog) + a cannot-answer-from-repo list. **The old app is NOT in this repo, so the packet is PREPARED but nothing was inspected live — this PR prepares direct old-app inspection; it does not complete it; direct old-app inspection is still required.** **Old-app parity / UI-UX / AI-API-connector parity NOT complete; hosted Auth/tenant-context verified but old-app replacement not yet verified; no feature built; no doc 17 box ticked; RISK-001 stays OPEN; cutover BLOCKED; not production-ready; Storage completion is necessary but not sufficient**) is this PR.** The full
