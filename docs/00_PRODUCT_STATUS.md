...
## Merged PRs
**PRs #1–#77 are merged** (main @ `8c190a2`); **PR #78 (harden contract-file upload finalization — feature hardening (E09a) closing the gap the PR #77 staging finding exposed (3 `files` rows for the contract, 1 orphan with no object, all stuck `pending`; doc 41 §12). PR #76 inserted the row first then uploaded the object, but `0013` had no UPDATE policy + `0015` granted only SELECT/INSERT, so a row could never leave `pending`. **Migration `0016`** (the narrow migration the existing schema required) adds a NARROW uploader-finalize UPDATE policy (`uploaded_by=auth.uid() AND can_write_contract`; WITH CHECK no-reassign; **column-scoped `grant update (upload_status)` only**) — proven by **T36** (suite 222→**227**). The DAL finalizes success → `upload_status='uploaded'` and dispositions a failed upload → `failed`; download only signs finalized rows; the UI shows Open only for finalized files. **No service-role on request paths, no public bucket, no DELETE/`FOR ALL`/broad grant; types 0-diff. Contract-file upload finalization is hardened; successful uploads no longer remain ambiguous pending rows; failed uploads are explicitly dispositioned. Storage authorization remains necessary but not sufficient for cutover; upload is not automatically production-ready; Invoices remain not built; PDF/AI extraction remains not built; Old-app/UI-UX/AI-API-connector parity NOT complete; no doc 17 box ticked; RISK-001 stays OPEN; cutover BLOCKED**) is this PR.** The full
