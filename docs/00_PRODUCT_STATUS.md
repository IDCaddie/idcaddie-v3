...
## Merged PRs
**PRs #1–#66 are merged** (main @ `7d3d388`); **PR #67 (document OMC acceptance signoff plan — docs-only (doc 36) for blocker-sequence item #6, the final item (doc 17 §5 box 17): acceptance definition, 8 signoff domains, signers-by-role, evidence package, 4 outcomes, approved-removal recording, hard rules. With this, all six blocker-sequence items (#1–#6) exist as prepared plans. **Planning only — no acceptance/signoff recorded, no hosted/staging/production mutation, no real OMC data, no secrets; RISK-001 stays OPEN; cutover BLOCKED; not production-ready; Storage completion is necessary but not sufficient**) is this PR.** The full
