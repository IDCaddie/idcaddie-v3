...
repo on 2026-06-17 (PRs through #42 merged — contract-file validation foundation + the Storage local-harness spike + the staging bucket apply runbook; #43 is a **docs-only evidence template** ([23_STORAGE_STAGING_APPLY_EVIDENCE_TEMPLATE](./23_STORAGE_STAGING_APPLY_EVIDENCE_TEMPLATE.md)): the fill-in record a human captures **while executing doc 22** in staging — metadata, pre-apply checklist, redacted apply evidence, the doc 21 §6 verification as 20 reviewer-signed rows, failure log, rollback, signoff. **TEMPLATE ONLY — NOTHING APPLIED**; no hosted mutation, no bucket, no policy, no upload action, no secrets; does NOT close RISK-001 or authorize cutover. RLS stays 205; tests 67/67; `files` still not surfaced; cutover stays BLOCKED; RISK-001/002/007/016 open). **OMC is a paying production-replacement customer, NOT a pilot.** Status words are defined in [10_DOCS_INDEX](./10_DOCS_INDEX.md#status-taxonomy).
...
## Merged PRs
**PRs #1–#42 are merged** (main @ `e97e9d1`); **PR #43 (Storage staging apply evidence template — docs-only, creates doc 23; nothing applied) is this PR.** The full
