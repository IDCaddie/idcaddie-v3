...
repo on 2026-06-17 (PRs through #41 merged — contract-file validation foundation + the Storage local-harness spike; #42 is a **docs-only runbook** ([22_HOSTED_STORAGE_BUCKET_APPLY_RUNBOOK](./22_HOSTED_STORAGE_BUCKET_APPLY_RUNBOOK.md)): the exact reviewed, human-executed **STAGING** apply + verification steps for the private `contract-files` bucket + object policies (the gate before any upload action ships). **Applies/creates NOTHING** — no hosted mutation, no bucket, no policy, no upload action; instantiates doc 20 + executes the doc 21 §6 checklist. RLS stays 205; tests 67/67; `files` still not surfaced; cutover stays BLOCKED; RISK-001/002/007/016 open). **OMC is a paying production-replacement customer, NOT a pilot.** Status words are defined in [10_DOCS_INDEX](./10_DOCS_INDEX.md#status-taxonomy).
...
## Merged PRs
**PRs #1–#41 are merged** (main @ `3baa761`); **PR #42 (hosted-staging contract-files bucket apply runbook — docs-only, creates doc 22; nothing applied) is this PR.** The full
