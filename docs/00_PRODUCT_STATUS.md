...
## Merged PRs
**PRs #1–#84 are merged** (main @ `dcb3fd6`); **PR #85 (improve people/identity read parity — UI/read-only (roadmap E05/E06, doc 41 §20): a read-only **`/people`** route + DAL `listIdentityAccountsForCurrentUser()` shows the identity ACCOUNTS the user may read (app_users across visible apps, each with app link + the account's own fields + matched/unmatched status + a summary: accounts, distinct apps, matched, unmatched). **Critical posture: reads `app_users`/`apps`/`matches` ONLY — never `people`/`identity_accounts` — so NO person PII / no `person_id`; the org-scoped people directory read stays deferred (RISK-002).** People/Users sidebar now enabled → `/people`; Identity matching stays "Not built yet" (read-only match status surfaced; resolution workflow not built). No migration/policy/RLS change; no writes; no cross-tenant (T29/T30); DTO has no tenant/person id (tested); auth guard preserved; types 0-diff; suite 248; tests 87→92 (incl. a nav test that every enabled nav item maps to an implemented route; build now 11 routes). **People/Users read-only parity is improved but not complete; manual/bulk matching + SCIM/IdP import + connector sync + AI matching + exports remain not built; Old-app/UI-UX/AI-API-connector parity NOT complete; hosted Auth/tenant-context verified but old-app replacement not yet verified; upload not production-ready; no doc 17 box ticked; RISK-001 stays OPEN; cutover BLOCKED**) is this PR.** The full
