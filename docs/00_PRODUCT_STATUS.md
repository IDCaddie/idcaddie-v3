...
## Merged PRs
**PRs #1–#85 are merged** (main @ `2b448e4`); **PR #86 (correct apps populated staging verification record — corrective docs-only (doc 41 §19 banner + §19.5): new live staging evidence shows `/apps` **empty ("No apps to show")** for `tenant-editor-a@idcaddie-staging.local` (tenant Storage Verifier Tenant A · editor), so the **PR #84 populated-path Apps verification is SUPERSEDED / CORRECTED — not deleted** (original record retained with a banner). **What is true: PR #83 code merged successfully + local validation passed; the `/apps` empty-state staging verification passed; Apps inventory/detail parity is improved IN CODE but populated hosted verification remains incomplete.** **NOT currently verified: populated-path `/apps` is currently NOT reproducible for this account (still shows "No apps to show"); the `/apps/[id]` populated detail path is not currently verified; the `/people` populated path is not currently verified.** The synthetic fixture is **either not present, not visible through RLS, or not associated with this account/tenant — exact root cause UNVERIFIED** (this corrective PR ran no hosted query). **No production data touched; no hosted commands run in this corrective PR; no RLS policies changed; no migrations added; does not claim the populated Apps/People paths passed; does not say old-app replacement is verified; old-app parity not complete; RISK-001 stays OPEN; cutover BLOCKED**) is this PR.** The full
