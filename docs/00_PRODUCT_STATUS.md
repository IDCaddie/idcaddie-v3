...
## Merged PRs
**PRs #1–#80 are merged** (main @ `1af5e3c`); **PR #81 (improve authenticated shell navigation — UI-only (roadmap epic E01, doc 41 §16): adds a persistent authenticated sidebar (`nav.tsx` + pure tested `nav-items.ts` wired into `layout.tsx`) across all authenticated routes with full old-app-parity nav groups, **active state**, and the signed-in email + active tenant name/role (**never the tenant id**) + sign-out. **Core shell/navigation UI parity is improved but not complete** — only the 3 implemented routes (`/`, `/apps`, `/contracts`) are linkable; **unbuilt old-app areas remain clearly marked as "Not built yet"** (disabled; no placeholder routes; no implication unbuilt modules work — a test fences that only real routes are linkable). No migration/policy/RLS/data-model/DB-write/hosted change; session guard unchanged; no secrets/IDs/tokens exposed; types 0-diff; suite 248; tests 77→83. **Old-app/UI-UX/AI-API-connector parity NOT complete; Storage authorization remains necessary but not sufficient for cutover; upload not production-ready; hosted Auth/tenant-context verified but old-app replacement not yet verified; no doc 17 box ticked; RISK-001 stays OPEN; cutover BLOCKED**) is this PR.** The full
