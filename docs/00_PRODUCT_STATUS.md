...
repo on 2026-06-17 (PRs through #39 merged — docs 17–20 gate/confirmation/vault/apply-discipline; #40 adds the **contract-file PDF validation foundation** — `src/lib/files/pdf-validation.ts` (server-side validation + server-derived tenant-bound storage path, pure + 16 unit tests; bucket `contract-files`). **No Storage bucket created, no upload action/UI, no signed URLs, no AI/OCR, no migration, no RLS change, no service-role, no hosted apply.** RLS stays 205; tests 51 → 67; `files` still not surfaced; cutover stays BLOCKED; RISK-001/002/007/016 open). **OMC is a paying production-replacement customer, NOT a pilot.** Status words are defined in [10_DOCS_INDEX](./10_DOCS_INDEX.md#status-taxonomy).
...
## Merged PRs
**PRs #1–#39 are merged** (main @ `b5f6c3f`); **PR #40 (private contract-file Storage validation foundation — `src/lib/files/pdf-validation.ts` + 16 tests; no bucket/upload/migration) is this PR.** The full
