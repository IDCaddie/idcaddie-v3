...
repo on 2026-06-17 (PRs through #51 merged — the contract-file Storage paper trail + the staging env-var inventory; the staging hosted apply is now **partial**: migrations `0001`–`0014` + the private `contract-files` bucket (`public=false`, 25 MiB, `application/pdf`) + the `storage.objects` **object policies are applied to the STAGING project `ycdpzduxugdsffjqyoai`** (2 `authenticated` INSERT/SELECT, 0 unsafe — **structural verification passed**) — [25 §0/§0.2](./25_STAGING_SCHEMA_AND_STORAGE_APPLY_EVIDENCE.md); #52 records that apply. **The real Storage REST API authorization verification (doc 21 §6) is still PENDING; production `dzbfxulvxchdemcettrx` NOT touched** → no upload route/action/UI, signed URLs, AI, or OCR built. **RISK-001 stays OPEN** (structural apply ≠ live authz; criterion 1 done, criterion 2 pending). RLS now 222; tests 67/67; `files` still not surfaced; cutover stays BLOCKED; RISK-001/002/007/016 open). **OMC is a paying production-replacement customer, NOT a pilot.** Status words are defined in [10_DOCS_INDEX](./10_DOCS_INDEX.md#status-taxonomy).
...
## Merged PRs
**PRs #1–#51 are merged** (main @ `795a50e`); **PR #52 (record staging Storage object-policy apply evidence — docs-only; a human applied the `contract-files` `storage.objects` policies in **staging** only, structural verification passed, **real REST authz verification still pending, no production change, RISK-001 stays OPEN**) is this PR.** The full
...
| Migrations applied to hosted Supabase (staging/prod) | **staging done** (`0001`–`0014` `staged` to `ycdpzduxugdsffjqyoai`; private `contract-files` bucket + `storage.objects` object policies applied — structural verification passed; **real REST authz verification still pending** — [25 §0.2](./25_STAGING_SCHEMA_AND_STORAGE_APPLY_EVIDENCE.md)); **production not done**. RISK-001 OPEN (partial — [04](./04_RISK_REGISTER.md)) |
