...
## Merged PRs
**PRs #1–#70 are merged** (main @ `1729a71`); **PR #71 (audit existing parity documentation — docs-only stocktake (doc 37) before another parity plan: a per-doc summary of all parity/cutover/readiness docs, separating **completed evidence** (staging+prod Storage REST 14/14; hosted Auth+tenant-context PASSED; Auth cleanup; the RLS unsafe-run gate) from **planned-not-executed** (docs 32/33/34/35/36 + doc 18 confirmation + doc 19 vault + doc 16 PDF/AI), the **missing/under-specified** parity areas vs the legacy feature list, **docs-drift** (parity tracked in 5 places → 27/17/33 canonical), a next-PR sequence (run the doc 18 confirmation pass first to size scope), and a bottom-line. Measured: doc 17 §5 = 0/17 boxes; doc 27 ≈ 111 missing / 58 partial / 67 blocked. **Old-app parity / UI-UX / AI-API-connector parity NOT complete; hosted Auth/tenant-context verified but old-app replacement not yet verified; audit implements no parity; no doc 17 box ticked; RISK-001 stays OPEN; cutover BLOCKED; not production-ready; Storage completion is necessary but not sufficient**) is this PR.** The full
