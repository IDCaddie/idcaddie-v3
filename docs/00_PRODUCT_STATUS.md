...
## Merged PRs
**PRs #1–#69 are merged** (main @ `bd93972`); **PR #70 (prepare hosted staging RLS verification runner — script + docs only; adds `scripts/verify-staging-rls-suite.mjs`, a staging-ref-guarded refusal gate for the remaining boxes 5/8 sub-task (full `org_rls_test.sql` re-run against hosted). The raw suite `TRUNCATE`s 17 tables incl. append-only `audit_logs` + `delete from auth.users`, so it must never run against shared hosted staging even rollback-only; the gate hard-refuses production, requires staging linked, detects the destructive statements and refuses the raw run; the script connects to nothing and only an explicit disposable-isolated opt-in **emits a rollback-only runbook** (count-snapshot → `begin…rollback` → verify counts unchanged → dispose) for a human to run against a separate disposable project, no secrets. **Hosted staging RLS execution is prepared but not yet run; no hosted command run; no production touched, no secrets; RISK-001 stays OPEN; cutover BLOCKED; not production-ready; Storage completion is necessary but not sufficient**) is this PR.** The full
