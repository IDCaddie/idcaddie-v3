...
- **Never mutate hosted Supabase** (no `supabase db push --linked`, no hosted apply, no schema/data change) **unless following [20_STAGING_HOSTED_APPLY_AND_CUTOVER_DISCIPLINE](./20_STAGING_HOSTED_APPLY_AND_CUTOVER_DISCIPLINE.md) AND explicitly approved by the human** — staging-first, clean tree, expected-migrations-only, verify-after-apply, never on type-drift/duplicate/non-green-RLS. An agent never initiates a hosted apply or production deploy on its own. RISK-001 is **open** — discipline ≠ apply. **⚠️ CAUTION (PR #45 finding):** the local Supabase CLI was found **linked to PRODUCTION** (`dzbfxulvxchdemcettrx`); the staging hosted apply is **still pending a human execution** (evidence: [25_STAGING_SCHEMA_AND_STORAGE_APPLY_EVIDENCE](./25_STAGING_SCHEMA_AND_STORAGE_APPLY_EVIDENCE.md)). Before ANY `--linked` command, a human must re-link to staging (`ycdpzduxugdsffjqyoai`) and verify `supabase/.temp/project-ref` — a `--linked` push while linked to production would be a production incident.
