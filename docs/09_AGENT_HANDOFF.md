...
- **Never mutate hosted Supabase** (no `supabase db push --linked`, no hosted apply, no schema/data change) **unless following [20_STAGING_HOSTED_APPLY_AND_CUTOVER_DISCIPLINE](./20_STAGING_HOSTED_APPLY_AND_CUTOVER_DISCIPLINE.md) AND explicitly approved by the human** — staging-first, clean tree, expected-migrations-only, verify-after-apply, never on type-drift/duplicate/non-green-RLS. An agent never initiates a hosted apply or production deploy on its own. RISK-001 is **open** — discipline ≠ apply. **⚠️ Staging status (PR #48, evidence in [25_STAGING_SCHEMA_AND_STORAGE_APPLY_EVIDENCE](./25_STAGING_SCHEMA_AND_STORAGE_APPLY_EVIDENCE.md) §0):** a **human** has applied migrations `0001`–`0013` to the **staging** project `ycdpzduxugdsffjqyoai` and created the private `contract-files` bucket (`public=false`, 25 MiB, `application/pdf`); **production `dzbfxulvxchdemcettrx` was NOT touched.** The local CLI is now re-linked to staging — **always `cat supabase/.temp/project-ref` and confirm `ycdpzduxugdsffjqyoai` (NOT `dzbfxulvxchdemcettrx`) before ANY `--linked` command; a `--linked` push against production is a production incident.** **Storage object policies are NOT yet applied** (`pg_policies` for `storage.objects` matching `contract%` = 0) — the **exact apply plan is now finalized in [22 §5](./22_HOSTED_STORAGE_BUCKET_APPLY_RUNBOOK.md)** (INSERT only when files metadata exists + contract-write authority; SELECT only when metadata readable; no UPDATE/DELETE/`FOR ALL`/public/cross-tenant; two `SECURITY DEFINER` helpers via a future tested migration `0014`, then staging-only policy apply). The only sanctioned next hosted mutation is that reviewed Storage object-policy apply (**staging only, explicit human approval, doc 22 §5**); an agent still never runs it. RISK-001 stays OPEN (partial staging apply); cutover BLOCKED.
