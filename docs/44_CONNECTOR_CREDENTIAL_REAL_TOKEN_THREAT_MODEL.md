...
| **PR B2c — SPLIT into three (never one PR)** | B2c is split so each trust boundary gets its own review/authorization: | |
| &nbsp;&nbsp;↳ **B2c-wire (✅ DONE, #176)** | **Synthetic callback composition** — `oauth-callback-orchestrator.ts` composes B2a validate (gate) → B2b mocked exchange → B1 store, threading the validated payload as the single source of truth; `b1StoreHandoff` wires the real B1 ingestion (token→ciphertext). **Mocked Slack, synthetic token, mocked client secret — no real call/token/secret.** | **No — synthetic.** |
