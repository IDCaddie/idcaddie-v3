...
| `0016_files_uploader_finalize_update.sql` | **Files uploader-finalize UPDATE policy** — a NARROW UPDATE so the **uploader** can finalize their OWN contract-file row's disposition after the Storage object upload (the gap PR #76 left; staging found `pending`/orphan rows — doc 41 §11/§12). Policy `"uploader finalizes own file"`: USING + WITH CHECK = `uploaded_by = auth.uid() AND can_write_contract(contract_id, tenant_id)` (uploader-only, contract-write authority; `uploaded_by`/`tenant_id`/`contract_id` cannot be reassigned). Column-scoped `grant update (upload_status) on public.files to authenticated` — ONLY `upload_status` (pending → `uploaded`|`failed`), never path/sha256/filename/contract/tenant/uploader. **NO DELETE, NO `FOR ALL`, NO new Storage policy, NO service-role, NO bucket/column change.** App finalizes a success to `uploaded` and dispositions a failed object upload to `failed`. Proven by **T36** (the column-grant narrowing is hosted-only, masked locally by the blanket grant — same class as `0015`; T34d's stale "no UPDATE" assertion superseded). RLS suite **222 → 227**. `database.types.ts` 0-diff (policy/grant only). **RISK-001 stays OPEN.** | PR #78 |
