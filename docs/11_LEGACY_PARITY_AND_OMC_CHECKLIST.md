...
| Contract upload / storage | Firebase Storage `/files/{id}` + metadata | `webapp/functions/src/storage/*`, `files/page.tsx` | none built — **DESIGNED** ([16](./16_CONTRACT_PDF_AI_EXTRACTION_DESIGN.md), PR #33): private bucket, tenant-bound paths, signed URLs, default-deny `files` RLS | Stage 5b⁵ design / Stage 8 build | tenant-scoped file storage | scoped storage policies (designed; not built) | `design` (not built; RISK-002 open) |
