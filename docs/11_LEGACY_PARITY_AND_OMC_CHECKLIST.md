...
| App–contract linking | `linkedDocs.IDCApps` cross-refs, cost allocation % | `contracts/[id]/page.tsx`, `IDCApps/[id]/page.tsx` | `app_contracts` table + RLS | Stage 5 | linked apps↔contracts | tenant-bound FKs; **not surfaced — `app_contracts` tenant-only, needs org-scoped read (RISK-002)** | `not-started` |
