...
| Tenant / company / org model | "Groups" + one Firebase project per customer; custom claims | `company/groups/`, `webapp/.firebaserc`, `permissionSync.js` | tenants + organizations + memberships + RLS; context resolved | done (PR #1/#9) | one tenant = one OMC org tree | row-level isolation (205 RLS assertions, T1–T34) vs per-project | `verified` |
...
| Contracts | List/detail/create/edit; renewal & expiry dates; gantt | `contracts/page.tsx`, `contracts/[id]/`, `contracts/create/`, `contracts/gantt/` | `/contracts` + `/contracts/[id]` read-only (PR #19) **+ create/edit** `/contracts/new` + `/contracts/[id]/edit` (PR #31) on the RLS-gated #30 write path, audited (`0010`); parity fields `category`/`procurement_date`/`notes`/`po_number`/`auto_renew`/`month_to_month` added (PR #32, `0011`). Write authority `0004`; `tenant_id` server-resolved. **Still Partial parity** — legacy `commodity_*` (hidden) + `validated` (read-only) + **gantt** have no v3 surface; **PDF/AI is DESIGNED ([16](./16_CONTRACT_PDF_AI_EXTRACTION_DESIGN.md), PR #33) but not built** ([15](./15_LEGACY_CONTRACT_FORM_INSPECTION.md)). No delete/archive/link/files | Stage 5 | OMC sees + can edit contracts | RLS related-org read + steward/editor write (verified); `paying_org_id` read-only | `partial` (read-only + create/edit incl. parity fields; PDF/AI `design`; gantt/delete/links `not-started`) |
...
| Contract upload / storage | Firebase Storage `/files/{id}` + metadata | `webapp/functions/src/storage/*`, `files/page.tsx` | **DESIGNED** ([16](./16_CONTRACT_PDF_AI_EXTRACTION_DESIGN.md), #33) + **`files` metadata schema** (`0012`, #34) + **`files` RLS policies** (`0013`, #35 — tenant-member SELECT + contract-write INSERT, no UPDATE/DELETE/FOR ALL); **no upload/Storage/signed-URL/UI surface yet** | Stage 5b⁵ design + 5b⁶ schema + 5b⁷ RLS / build later | tenant-scoped file storage | tenant-bound metadata + FK + tested RLS live; `files` still not app-surfaced | `design + schema + RLS` (`files` not surfaced; RISK-002 open) |
| AI contract ingestion / extraction | Google Document AI (contract/invoice processors) + Vertex AI/Gemini fallback | `webapp/functions/src/storage/processFileWithAI.js`, `handleDocumentAICompletion.js` | none built — **DESIGNED** ([16](./16_CONTRACT_PDF_AI_EXTRACTION_DESIGN.md), PR #33): AI **suggestions only**, strict allowlist, no service-role app route, user reviews+applies | Stage 5b⁵ design / build later | extract renewal/cost fields | provider boundary; AI output untrusted, validated by `parseContractWriteInput`; no secrets in app tables | `design` (not built) |
...
| 3 | Can OMC see contracts and renewal dates? | **Partial** — read-only list + detail (PR #19) **+ create/edit UI** (PR #31, RLS-gated, audited) **+ parity fields** category/procurement_date/notes/po_number/auto_renew/month_to_month (PR #32, `0011`); still **Partial parity** — `commodity_*`/`validated`/PDF-AI/gantt/delete missing ([15](./15_LEGACY_CONTRACT_FORM_INSPECTION.md)) | Stage 5 — read + create/edit + parity fields shipped; PDF-AI/gantt later |
