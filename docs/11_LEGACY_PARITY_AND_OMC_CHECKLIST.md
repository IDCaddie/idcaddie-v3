...
| Tenant / company / org model | "Groups" + one Firebase project per customer; custom claims | `company/groups/`, `webapp/.firebaserc`, `permissionSync.js` | tenants + organizations + memberships + RLS; context resolved | done (PR #1/#9) | one tenant = one OMC org tree | row-level isolation (186 RLS assertions, T1–T33) vs per-project | `verified` |
...
| Contract upload / storage | Firebase Storage `/files/{id}` + metadata | `webapp/functions/src/storage/*`, `files/page.tsx` | **DESIGNED** ([16](./16_CONTRACT_PDF_AI_EXTRACTION_DESIGN.md), PR #33) + **`files` metadata schema foundation built** (`0012`, PR #34 — columns + same-tenant `contract_id` FK + CHECKs); **no upload/Storage/signed-URL/RLS surface yet** | Stage 5b⁵ design + 5b⁶ schema / build later | tenant-scoped file storage | tenant-bound metadata + same-tenant FK live; `files` RLS still default-deny (designed, not built) | `design + schema` (`files` not surfaced; RISK-002 open) |
