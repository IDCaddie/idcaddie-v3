...
- **Contract-file upload finalization is hardened. Successful uploads no longer remain ambiguous pending rows. Failed uploads are explicitly dispositioned or documented as blocked by current policy.**
  - **Migration `0016`** — a NARROW UPDATE policy `"uploader finalizes own file"` (USING + WITH CHECK = `uploaded_by = auth.uid() AND can_write_contract(contract_id, tenant_id)`; no `uploaded_by`/`tenant_id`/`contract_id` reassignment) + a **column-scoped** `grant update (upload_status) on public.files to authenticated` (ONLY `upload_status`). No DELETE/`FOR ALL`, no Storage-policy/bucket change, no service-role.
...
- **Security/tests:** new **T36** in `org_rls_test.sql` proves the uploader can finalize their own row but cross-tenant / cross-user updates and `uploaded_by`/`tenant_id` reassignment are denied (the column-grant narrowing is hosted-only, masked locally by the blanket grant — same class as `0015`; the stale T34d "no UPDATE" assertion was superseded). RLS suite **222 → 227**. +1 app-layer test (77 total): finalize sets `uploaded`; failed disposition sets `failed`; download refuses non-finalized rows; DTO still never leaks `storage_path`. An adversarial security review of `0016` was run.
...
- **Validation (local, verified):** `npm test` **77/77**; lint clean; `tsc --noEmit` clean; `next build` clean (10 routes); `check-auth-safety`/`check-migration-safety`/`check-docs-updated`/`pr-review-summary` pass; `test-rls.sh` **227**; `gen-types-local.sh` 0-diff (policy/grant only); `node --check` on all three scripts OK; no `* 2.*`/`* 3.*` strays.
