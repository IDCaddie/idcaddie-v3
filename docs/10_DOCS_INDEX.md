...
| [25_STAGING_SCHEMA_AND_STORAGE_APPLY_EVIDENCE](./25_STAGING_SCHEMA_AND_STORAGE_APPLY_EVIDENCE.md) | **dated staging-apply evidence (PR #47 + #48, 2026-06-17)** — §0: a **human applied migrations `0001`–`0013` to staging `ycdpzduxugdsffjqyoai` + created the private `contract-files` bucket** (`public=false`, 25 MiB, `application/pdf`); **Storage object policies NOT yet applied; production NOT touched.** §1–§6: the PR #47 agent session (correctly did NOT execute). **No upload/UI/AI built; RISK-001 stays OPEN (partial staging apply); cutover blocked; no secrets.** | ops/release owner | evidence (object policies + verification still pending) |
