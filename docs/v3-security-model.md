...
> - Org scoping for child-table **reads** is done (read-only) for `app_contracts` (`0006`, PR #20 — read a link iff you can read the linked app or contract) and `app_users` (`0007`, PR #21 — read an app-user iff you can read its app). It remains deferred for `people` (tenant-only) and the default-deny tables (`license_*`, `files`, `invoices`, `identity_*`). A future `resource_org_links` table + org hierarchy may supersede the column-based model. Canonical read map: [02 §8](./02_SECURITY_AND_RLS.md).
