...
- ❌ Surfacing a **default-deny** child table (`identity_accounts`/`app_user_identity_matches`/`license_rules`/`license_evaluations`/`files`/`invoices`) or a **tenant-only** child table (`people`/`app_users`) to org-only users **as if it were org-scoped** — `0005` FKs protect *writes*, not reads; an org-scoped read policy + test must land first (RISK-002; canonical read map [02 §8](./02_SECURITY_AND_RLS.md)). (`app_contracts` is already org-scoped for read — `0006`/§8a, T28 — and may be surfaced read-only.)
