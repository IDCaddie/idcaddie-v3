...
- ❌ Surfacing a **default-deny** child table (`identity_accounts`/`app_user_identity_matches`/`license_rules`/`license_evaluations`/`files`/`invoices`) or a **tenant-only** child table (`people`/`app_users`/`app_contracts`) to org-only users **as if it were org-scoped** — `0005` FKs protect *writes*, not reads; an org-scoped read policy + test must land first (RISK-002; canonical read map [02 §8](./02_SECURITY_AND_RLS.md), pinned by T27).
