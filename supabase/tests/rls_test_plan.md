...
correlation, malformed workspace, unsupported scheme, undersized and missing payload, out-of-grammar correlation — and
not one refusal left a row · J4 idempotency is keyed on the REQUEST: identical bytes return the same job, a substituted
...
consumed no pending row · J8 expiry: an expired job is unclaimable and uncompletable, the discovery path retires it and
