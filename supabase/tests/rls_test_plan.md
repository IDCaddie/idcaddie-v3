...
**T53 = the `connector_secret_lifecycle_events` SELECT-only runner grant + APPEND-ONLY trigger (`0032`, PR #169)** — proves TWO SEPARATE protections: (1) the runner GRANT SHAPE under `set role connector_runner` — column-scoped SELECT on EXACTLY (tenant_id, connector_id, secret_kind, version, lifecycle_event_type); INSERT/UPDATE/DELETE all FAIL (NO runner INSERT grant this PR); (2) the append-only TRIGGER itself, proven against a PRIVILEGED test/admin role that CAN attempt UPDATE/DELETE and is STILL rejected by the trigger (the `append-only` error — so the block is the trigger, not grant-absence); plus a functional proof that the lifecycle-aware load EXCLUDES an ACTIVE version carrying a `revoked` event, and that `connector_secrets` keeps no runner UPDATE/DELETE;
