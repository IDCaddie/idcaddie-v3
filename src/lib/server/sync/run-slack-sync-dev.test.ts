...
describe("manual run is server-only, with no public route / server action / UI trigger added", () => {
  ...
  const modules = ["run-slack-sync-dev.ts", "dev-user-scoped-client.ts", "supabase-slack-resolver-store.ts", "slack-fetch-http-client.ts", "manual-sync-run-recorder.ts", "recorded-slack-sync-run.ts"];
