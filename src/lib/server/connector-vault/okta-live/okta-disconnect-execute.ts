// P5E18b — the staging-safe DISCONNECT EXECUTION (Phase 10). Applies the pure disconnect PLAN (P5E18a planOktaDisconnect) through
// server-only injected sinks: mark the connection disconnected, invalidate execution eligibility immediately, invalidate pending
// OAuth transactions, disable any schedule, and revoke the credential reference through a server-only interface. Audit history is
// preserved; it is idempotent; it never reveals a secret/credential identifier to the client. NO real Okta revocation endpoint is
// called (the sink's provider-side revocation is documented as a future requirement).
//
...
import type { OktaTransactionStore } from "./okta-transaction-store";
...
export interface OktaDisconnectSinks {
  ...
  transactionStore: OktaTransactionStore;
}
...
export async function executeOktaDisconnect(
  input: OktaDisconnectInput & { pendingCorrelationIds?: readonly string[] },
  ...
  try {
    ...
    for (const cid of input.pendingCorrelationIds ?? []) {
      await deps.transactionStore.invalidate(cid, input.now, "disconnected");
    }
