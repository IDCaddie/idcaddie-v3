...
export default async function AccessOverviewPage({ searchParams }: { searchParams: Promise<{ stale?: string }> }) {
          ...
          {result.data.status === "too_large" ? (
            ...
            <>
              <section className="space-y-2">
                ...
                <StatGrid>
                  <StatCard label="Effective access relationships" value={result.data.effectiveRelationships} />
                  <StatCard label="Direct only" value={result.data.breakdown.directOnly} />
                  <StatCard label="Through group only" value={result.data.breakdown.groupOnly} />
                  <StatCard label="Direct and through group" value={result.data.breakdown.both} />
                </StatGrid>
              </section>

              <section className="space-y-2">
                <div className="flex items-center justify-between">
                  <h2 className="text-sm font-medium">Governance findings ({result.data.governanceFindingsTotal})</h2>
                  <Link href={includeStale ? "/access/findings?stale=1" : "/access/findings"} className="text-xs underline">
                    View all findings
                  </Link>
                </div>
                {result.data.findings.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {result.data.findings.slice(0, 10).map((f) => (
                      <li key={f.id} className="rounded border border-zinc-200 p-3 text-sm dark:border-zinc-800">
                        ...
                        {f.subject ? (
                          <Link href={f.subject.href} className="mt-1 inline-block text-xs underline">
